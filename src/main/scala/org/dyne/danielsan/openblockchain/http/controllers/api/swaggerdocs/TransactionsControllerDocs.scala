...
trait TransactionsControllerDocs extends SwaggerSupport {
  ...
  val getTransactions =
    (apiOperation[List[Transaction]]("getTransactions")
      ...
      notes "Currently shows all transactions, with no possibility of filtering them. TODO: add filtering functionality. Mock data can be found here: https://github.com/dan-mi-sun/openblockchain/blob/feature/webapp/src/main/resources/mocks/transactions.csv ")

  val getTransaction =
    (apiOperation[Transaction]("getTransaction")
      summary "Shows a transaction"
      notes "Retrieves a transaction by its txid. Mock data can be found here: https://github.com/dan-mi-sun/openblockchain/blob/feature/webapp/src/main/resources/mocks/transactions.csv"
      parameters
